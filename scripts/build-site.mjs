...
// and two machine-readable indexes of those same pages — sitemap.xml and llms.txt.
//
...
const readPlugin = async entry => {
  ...
  const servers = Object.entries(declaredServers).map(([name, config]) => {
    const configKey = userConfigReference(config.headers)
    return {
      ...config,
      name,
      credentialTitle: configKey ? manifest.userConfig?.[configKey]?.title : undefined
    }
  })
...
const llmsTxt = ({ catalog, plugins, siteUrl, repoUrl }) => {
...
/** A plugin page's visible breadcrumb, in schema.org form: the same two labels and the same
...
const breadcrumbJsonLd = (catalog, plugin, siteUrl) =>
  JSON.stringify(
    {
      ...
      itemListElement: [
        ...
        { '@type': 'ListItem', position: 2, name: plugin.display }
      ]
    },
    null,
    2
...
const build = async () => {
    ...
    plugins.map(plugin => {
      const html = render(
        ...
        partials,
        {
          ...
          BREADCRUMB_JSONLD: breadcrumbJsonLd(catalog, plugin, siteUrl)
        },
  ...
  const pageUrls = [indexUrl, ...pluginUrls]
  await writeOutput('sitemap.xml', sitemapXml(pageUrls))
  await writeOutput('llms.txt', llmsTxt({ catalog, plugins, siteUrl, repoUrl }))
